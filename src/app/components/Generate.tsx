...
export function Generate() {
  ...
  const buildTeacherPreferences = () => {
    const preferencesFromWords =
      styleProfile.favoriteWords.length > 0
        ? [`Mots a privilegier: ${styleProfile.favoriteWords.join(", ")}`]
        : [];
  ...
  const generateForStudent = async (studentId: number) => {
    ...
    if (quotaStatus.remaining <= 0) {
      setInfoMessage("Quota quotidien atteint (20 appels API). Revenez demain.");
      return;
    }
  ...
  const generateAllSelected = async () => {
    ...
    if (!canUseGeminiApi()) {
      setInfoMessage("API Gemini non configuree. Verifiez VITE_PRONOTEBOOST_API_URL.");
      return;
    }

    if (studentsToGenerate.length > 30) {
      setInfoMessage("Maximum 30 eleves par requete batch.");
      return;
    }

    setIsGenerating(true);
    setInfoMessage(`Appel IA batch en cours pour ${studentsToGenerate.length} eleve(s)...`);
    ...
    try {
      ...
      setQuotaStatus(getDailyQuotaStatus());
      setInfoMessage(`Generation IA terminee pour ${batchResult.length} eleve(s).`);
    } catch (error) {
      setQuotaStatus(getDailyQuotaStatus());
      setInfoMessage((error as Error).message || "Echec de generation batch.");
      setIsGenerating(false);
      return;
    }
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="bg-gradient-to-br from-[#396155] to-[#2a4a40] rounded-lg px-4 py-2 flex items-center gap-2 shadow-md">
            ...
            <div className="text-white">
              <div className="text-xs font-medium">Restant / jour</div>
              <div className="text-lg font-bold">{quotaStatus.remaining} / 20</div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="mb-3 rounded-lg border border-gray-200 bg-white px-3 py-3">
            <div className="mb-2 text-xs text-gray-700">Style enseignant actif</div>
            <div className="space-y-1 text-[11px] text-gray-600">
              <p>Matière: {styleProfile.subject || "non renseignée"}</p>
              <p>Mots préférés: {styleProfile.favoriteWords.length}</p>
              <p>Phrases préférées: {styleProfile.favoritePhrases.length}</p>
            </div>
            <button
              onClick={() => navigate("/style")}
              className="mt-2 text-xs text-[#396155] underline hover:text-[#2a4a40]"
            >
              Modifier mon style
            </button>
          </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 shadow-lg">
          {selectedWithoutAppreciationCount > 0 ? (
            <button
              ...
              disabled={isGenerating || isInserting || isLoadingStudents || quotaStatus.remaining <= 0}
